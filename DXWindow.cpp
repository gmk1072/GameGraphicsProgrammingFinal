...
DXWindow* DXWindow::DXWindowInstance = nullptr;
...
{
	return DXWindowInstance->ProcessMessage(hWnd, uMsg, wParam, lParam);
}
...
HRESULT DXWindow::InitWindow()
{
	...
	windowLocation.x = centeredX;
	windowLocation.y = centeredY;
...
// --------------------------------------------------------
void DXWindow::CreateConsoleWindow(int bufferLines, int bufferColumns, int windowLines, int windowColumns)
{
...
// unresponsive.
// --------------------------------------------------------
LRESULT DXWindow::ProcessMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOVE:
		windowLocation = MAKEPOINTS(lParam);
		return 0;
...
// --------------------------------------------------------
// Get the width of the clients window area
// --------------------------------------------------------
unsigned const int& DXWindow::GetWidth() const
{
	return width;
}

// --------------------------------------------------------
// Get the height of the clients window area
// --------------------------------------------------------
unsigned const int& DXWindow::GetHeight() const
{
	return height;
}
